...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<div style="display: flex;">'.'<div class="checkbox"><input type="checkbox" value="' . $aRow['id'] . '"><label></label></div>
                ...
                <a class="btn btnIcon" data-toggle="tooltip" title="" data-original-title="'. _l('mailbox_delete').'" onclick="update_field(\''.$group.'\',\'trash\',1,'.$aRow['id'].');"><i class="fa fa-trash-o"></i></a>'.'</div>';
    
    $content = '<a href="'.admin_url().'mailbox/inbox/'.$aRow['id'].'">';
    $row[] = $content.'<span class="'.$read.'">'.$aRow['sender_name'].'</span></a>';
    $row[] = $content.'<span class="'.$read.'">'.$aRow['subject'].'</span>'.'<span class="text-muted" style="word-break: break-word;max-width: 30vw;overflow: hidden;display: block;height: 40px;">'.strip_tags($aRow['body']).'</span>'.$has_attachment.'</a>';    
