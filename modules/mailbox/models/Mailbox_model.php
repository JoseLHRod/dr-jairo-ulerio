    ...
    public function add($data, $staff_id, $ob_id = null)
    {
        ...
        if (strlen(get_option('smtp_host')) > 0 && strlen(get_option('smtp_password')) > 0 && strlen(get_option('smtp_username')) > 0) {
            ...
            if($ci->email->send(true)){
                log_activity('Email sent to: '. str_replace(';', ',', $data['to']));
            }else{
                log_activity('<h1>Your SMTP settings are not set correctly here is the debug log.</h1><br />' . $ci->email->print_debugger() . (isset($GLOBALS['debug']) ? $GLOBALS['debug'] : ''));
            }
        }
