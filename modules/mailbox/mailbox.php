        ...
        foreach ($staffs as $staff) {
            ...
            if (empty($last_run) || (time() > $last_run + ($check_every * 60))) {
                ...
                if (false === $imap->isConnected()) {
                    ...
                    log_activity('Failed to connect to IMAP from email: '.json_encode(imap_errors()), null);
                    continue;
                }

                log_activity('Connect to IMAP from email: '.$staff_email, null);
                ...
                log_activity('Messages get from email '.$staff_email.':'.count($emails), null);

                foreach ($emails as $email) {
                    $custom_from = $email['from'];
                    ...
                    if(strpos($custom_body, 'Prospective Patient Inquiry') !== false && strpos($custom_from, 'expert@email.realself.com') !== false){
                        preg_match('/Patient\s*name\:\s*([^\d]*)/mi', $inbox['body'], $lead_name);
                        preg_match('/Call\s*or\s*SMS:\s*(\+?(\d*\s?)*)/mi', $inbox['body'], $lead_phone);
                        preg_match('/Patient\s*email\:\s*(\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+)/mi', $inbox['body'], $lead_email);
                        
                        if(count($lead_name) > 0 && count($lead_phone) > 0 && count($lead_email) > 0){
                            $lead = [];
                            $lead['email'] = $lead_email[1];
                            $lead['name'] = preg_replace('/([\r\n]+|\<br\>)+/mi', "", $lead_name[1]);
                            $lead['phonenumber'] = preg_replace('/([\r\n]+|\<br\>)+/mi', "", $lead_phone[1]);
                            $lead['source'] = 4;
                            $lead['status'] = 15;
                            $lead['addedfrom'] = 1;
                            $lead['is_public'] = 1;
						...
						//$conn->insert 'phonenumber' => esc_sql($entry['3'])]);
                    }
