...
class Imap
{
    ...
    public function getUnreadMessages($withbody = true)
    {
        $emails = [];
        $date = date("j F Y");
        $m_gdmy = 'SINCE "'.(date('d') - 1).'-October-'.date('Y').'"';
        //$result = imap_search($this->imap, 'UNSEEN SUBJECT "prueba"');
        //$emails = imap_search($inbox,'UNSEEN ON "'.$date.'"' );
