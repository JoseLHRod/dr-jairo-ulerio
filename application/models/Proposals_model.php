...
class Proposals_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        $data  = $hook['data'];
        $data["mail_templateid"]  = isset($data['template_mail_id']) && !empty($data['template_mail_id']) ? $data['template_mail_id'] : 0;
        unset($data['template_mail_id']);
        $items = $hook['items'];
    ...
    public function update($data, $id)
    {
        ...
        if ($save_and_send === true) {
            ...
            $template = $this->emails_model->get_email_template_by_id($data["mail_templateid"]);
            $template->has_tracking = true;
            $template->subject = $data["subject"];
            $template->tmp_id = app_generate_hash();
            $template->rel_id = $id;
            $template->rel_type = 'proposal';
            
            hooks()->do_action('email_template_sent', ['template' => $template, 'email' => $data['email']]);*/
