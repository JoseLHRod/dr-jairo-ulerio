...
class Emails_model extends App_Model
{
     ...
     * Get email template by id
     */
    public function get_proposals_slug()
    {
        $this->db->where('type', 'proposals');
        $query = $this->db->get(db_prefix() . 'emailtemplates');
    ...
    public function update($data)
    {
        ...
        foreach ($data['subject'] as $id => $val) {
            ...
            $_data['show_on_proposal']    = $data['show_on_proposal'];
    ...
    /**
     * Change template to proposals
     * @param  string $slug    template slug
     * @param  mixed $enabled enabled or disabled / 1 or 0
     * @return boolean
     */
    public function show_on_proposals($slug, $enabled)
    {
        $this->db->where('slug', $slug);
        $this->db->update(db_prefix() . 'emailtemplates', ['show_on_proposal' => $enabled]);
