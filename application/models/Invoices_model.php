...
class Invoices_model extends App_Model
{
    ...
    const STATUS_DEPOSIT_PAID = 7;

    const STATUS_DEPOSIT_SENT_BY_PATIENT = 8;
    ...
    private $statuses = [
        ...
        self::STATUS_DEPOSIT_SENT_BY_PATIENT,
    ];
    ...
    public function update_status2($id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'invoices', [
            'status' => self::STATUS_DEPOSIT_SENT_BY_PATIENT,
        ]);
    }
    ...
    {
        $this->db->select('total, paymentamount');
        $this->db->where('id', $id);
        $query = $this->db->get(db_prefix() . 'invoices'); 
        $prices = $query->result();
        $new_price = $prices[0]->total - $prices[0]->paymentamount;
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'invoices', [
            'total' => $new_price,
        ]);
    ...
    {
        $this->db->where('id', $id);
        return $this->db->update(db_prefix() . 'invoices', [
    ...
    public function send_invoice_to_client2($id, $template_name = '', $attachpdf = true, $cc = '', $manually = false, $attachStatement = [])
    {
        $invoice = $this->get($id);
