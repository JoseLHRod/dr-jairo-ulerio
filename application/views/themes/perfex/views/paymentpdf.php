...
$pdf->writeHTMLCell(50, 50, 10, 10, '<img src="'.payment_gateway_logo_url().'" />');
$pdf->Ln(30);
...
$pdf->Ln(2);
$pdf->writeHTMLCell(80, '', '', '', '<hr/>', 0, 1, false, true, 'L', true);
$pdf->Cell(0, 0, 'Subtotal' . ': ' . app_format_money($invoice->subtotal, $invoice->currency_name), 0, 1, 'L', 0, '', 0);

$pdf->Ln(2);
$pdf->writeHTMLCell(80, '', '', '', '<hr/>', 0, 1, false, true, 'L', true);
$pdf->Cell(0, 0, 'Discounts' . ': ' . app_format_money($invoice->paymentamount, $invoice->currency_name), 0, 1, 'L', 0, '', 0);
...
$pdf->Cell(0, 0, 'Items', 0, 1, 'L', 0, '', 0);
$pdf->SetFont($font_name, '', $font_size);
$pdf->Ln(5);

$tblhtmlitems = '<table width="100%" bgcolor="#fff" cellspacing="0" cellpadding="5" border="0">';
$tblhtmlitems .= '<thead>';
$tblhtmlitems .= '<tr style="color:#fff;" bgcolor="#3A4656">';
$tblhtmlitems .= '<th width="70%" align="left">'. _l('invoice_table_item_heading').'</th>';
$tblhtmlitems .= '<th width="15%" align="right">'. _l('invoice_table_quantity_heading') .'</th>';
$tblhtmlitems .= '<th width="15%" align="right">'. _l('invoice_table_amount_heading').'</th>';
$tblhtmlitems .= '</tr>';
$tblhtmlitems .= '</thead>';
$tblhtmlitems .= '<tbody>';
...
foreach ($add_items as $item) {
    $tblhtmlitems .= '<tr class="item">';
    $tblhtmlitems .= '<td width="70%" class="bold description">' . clear_textarea_breaks($item['description']) . '<br />';
    $tblhtmlitems .= '<span style="font-weight: 300; font-size: 12px;">'.clear_textarea_breaks($item['long_description']) . '</span></td>';
    $tblhtmlitems .= '<td width="15%" align="right">'.$item['qty'].'</td>';
    $amount = app_format_money($item['rate'], $payment->invoice_data->currency_name);
    $tblhtmlitems .= '<td width="15%" align="right">' . $amount . '</td>';
    $tblhtmlitems .= '</tr>';
}

$tblhtmlitems .= '</tbody>';
$tblhtmlitems .= '</table>';
$pdf->writeHTML($tblhtmlitems, true, false, false, false, '');
...
$pdf->SetFont($font_name, '', $font_size);
$pdf->Ln(5);
$pdf->Cell(0, 0, $payment->invoice_data->terms, 0, 1, 'L', 0, '', 0);*/