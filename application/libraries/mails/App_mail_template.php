...
class App_mail_template
{
    ...
    public function prepare($email = null, $template = null, $params = [])
    {
        ...
        if($this->rel_type !== 'proposal') $template = $this->ci->emails_model->get(['language' => $language, 'slug' => $slug], 'row');
        else{
            $idtemplate = $this->proposal->mail_templateid;
            if($idtemplate == 0) $template = $this->ci->emails_model->get(['language' => $language, 'slug' => $slug], 'row');
            else $template = $this->ci->emails_model->get(['language' => $language, 'emailtemplateid' => $idtemplate], 'row');
            
            if (!$template)  $template = $this->ci->emails_model->get(['language' => 'english', 'emailtemplateid' =>  $idtemplate], 'row');
