...
function add_email_tracking($data)
{
    ...
    if ( in_array($data['template']->slug, get_available_tracking_templates_slugs()) && isset($data['template']->has_tracking) && $data['template']->has_tracking ) {
        $CI->db->insert(db_prefix().'tracked_mails', [
            'uid'      => $data['template']->tmp_id,
            'subject'  => $data['template']->subject,
            'rel_id'   => isset($data['template']->rel_id) ? $data['template']->rel_id : $GLOBALS['SENDING_EMAIL_TEMPLATE_CLASS']->get_rel_id(),
            'rel_type' => isset($data['template']->rel_type) ? $data['template']->rel_type : $GLOBALS['SENDING_EMAIL_TEMPLATE_CLASS']->get_rel_type(),
...
function get_available_tracking_templates_slugs()
{
    ...
    $slugs = array_unique(array_merge($slugs, $CI->emails_model->get_proposals_slug()));
