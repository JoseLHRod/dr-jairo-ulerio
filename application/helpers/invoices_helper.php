...
function format_invoice_status($status, $classes = '', $label = true)
{
    ...
    } elseif ($status == Invoices_model::STATUS_DEPOSIT_PAID) {
        $status = _l('deposit paid');
    } elseif ($status == Invoices_model::STATUS_DEPOSIT_SENT_BY_PATIENT) {
        $status = _l('deposit sent by patient');
    } else {
        // status 6
