...
class Proposals extends AdminController
{
    ...
    public function proposal($id = '')
    {
        ...
        $data['proposals_mails'] = $this->emails_model->get([
            'type'     => 'proposals',
            'language' => 'english',
        ]);
    ...
    public function get_proposal_data_ajax($id, $to_return = false)
    {
        ...
        $this->load->model('emails_model');
        $emailtemplate = $this->emails_model->get_email_template_by_id($proposal->mail_templateid);
        $this->app_mail_template->set_rel_id($proposal->id);
        $data = $this->emails_model->get_email_template_by_id($proposal->mail_templateid);
        
        if(empty($data)) $data = prepare_mail_preview_data('proposal_send_to_customer', $proposal->email);
        else{
            $tem_data = [];
            $tem_data["template"] = $data;
            $tem_data["template_name"] = $data->slug;
            $tem_data["template_system_name"] = $data->name;
            $tem_data["template_id"] = $proposal->mail_templateid;
            $tem_data["template_disabled"] = $data->active == 0;
            $data = $tem_data;
        }
    ...
    public function convert_to_invoice($id)
    {
        ...
        if ($this->input->post()) {
            ...
            if ($invoice_id) {
                ...
                $this->send_to_email2($invoice_id);                 
            } else {
    ...
    public function send_to_email2($invoice_id)
    {
        $canView = user_can_view_invoice($invoice_id);
        if (!$canView) {
            access_denied('Invoices');
        } else {
        ...
        try {
            ...
            $success = $this->invoices_model->send_invoice_to_client2(
                $invoice_id,
                '',
                true,
                '',
