...
class Patients extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['statuses'] = array_filter($data['statuses'], function($status){ return $status['statustype'] == 'patients'; });
    ...
    public function patient($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                // if the current client dont exist redirect to patients index page
                ...
                $this->_imageControl('ppassport_pic', $id, $client->ppassport_pic);
                $this->_imageControl('pfront_pic', $id, $client->pfront_pic);
                $this->_imageControl('pl_side_pic', $id, $client->pr_side_pic);
                $this->_imageControl('pr_side_pic', $id, $client->pl_side_pic);
                $this->_imageControl('pback_pic', $id, $client->pback_pic);
                $this->_imageControl('pfront_pic_after', $id, $client->pfront_pic_after);
                $this->_imageControl('pl_side_pic_after', $id, $client->pl_side_pic_after);
                $this->_imageControl('pr_side_pic_after', $id, $client->pr_side_pic_after);
                $this->_imageControl('pback_pic_after', $id, $client->pback_pic_after);
    ...
    {
        if (!has_permission('customers', '', 'view')) {
            if ($id != '' && !is_customer_admin($id) && $image != '') {
                access_denied('customers');
            }
        }
        ...
        //prepare the images for validation
        $currentImages = [
            'ppassport_pic' => $client->ppassport_pic,
            'pfront_pic' => $client->pfront_pic,
            'pr_side_pic' => $client->pr_side_pic,
            'pl_side_pic' => $client->pl_side_pic,
            'pback_pic' => $client->pback_pic,
            'pfront_pic_after' => $client->pfront_pic_after,
            'pl_side_pic_after' => $client->pl_side_pic_after,
            'pr_side_pic_after' => $client->pr_side_pic_after,
            'pback_pic_after' => $client->pback_pic_after
        ];

        // validate if the image exist and return the field in the DB will be use
        $inArray = array_search($image, $currentImages);
        if (!$inArray) {
            set_alert('danger', _l('problem_deleting', _l('image')));
            redirect(admin_url('patients/patient/' . $id));
        }
        ...
        if ($success == true) {
            // delete the image
            if (is_file("uploads/clients/client_" . $id . "/" . $image)) {
                $unlink = unlink("uploads/clients/client_" . $id . "/" . $image);

                if ($unlink == true) {
                    log_message('successful', 'delete file: ' . $image);
                } else {
                    log_message('error', 'cannot delete file: ' . $image);
                }
    ...
    public function assign_admins($id)
    {
        ...
        redirect(admin_url('patients/patient/' . $id . '?tab=customer_admins'));
    }
    ...
    public function delete_customer_admin($customer_id, $staff_id)
    {
        ...
        redirect(admin_url('patients/patient/' . $customer_id) . '?tab=customer_admins');
    }
    ...
    public function delete_contact($customer_id, $id)
    {
        ...
        redirect(admin_url('patients/patient/' . $customer_id . '?group=contacts'));
    }
    ...
    {            
        ...
        if ($this->input->post()) {
            $data = $this->input->post();
            $iduser = $data['iduser'];
            $success  = $this->cron_job_model->send_email_review($iduser);

            if ($success) {
                set_alert('success', _l('patient_sent_to_email_success'));
            } else {
                set_alert('danger', _l('patient_sent_to_email_fail'));
            }  
    ...
    public function zip_credit_notes($id)
    {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                ...
                'redirect_on_error' => admin_url('patients/patient/' . $id . '?group=credit_notes'),
            ]);
    ...
    public function zip_invoices($id)
    {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                ...
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('patients/patient/' . $id . '?group=invoices'),
            ]);
    ...
    public function zip_estimates($id)
    {
        ...
        if ($this->input->post()) {
            $this->load->library('app_bulk_pdf_export', [
                ...
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('patients/patient/' . $id . '?group=estimates'),
            ]);
    ...
    public function zip_payments($id)
    {
        ...
        $this->load->library('app_bulk_pdf_export', [
                ...
                'date_from'         => $this->input->post('zip-from'),
                'date_to'           => $this->input->post('zip-to'),
                'redirect_on_error' => admin_url('patients/patient/' . $id . '?group=payments'),
            ]);
    ...
    public function delete_group($id)
    {
        ...
        if (!$id) {
            redirect(admin_url('patients/groups'));
        }
    ...
    public function statement_pdf()
    {
        ...
        if (!has_permission('invoices', '', 'view') && !has_permission('payments', '', 'view')) {
            set_alert('danger', _l('access_denied'));
            redirect(admin_url('patients/patient/' . $customer_id));
        }
    ...
    public function send_statement()
    {
        ...
        redirect(admin_url('patients/patient/' . $customer_id . '?group=statement'));
    }
     ...
     * @return void
     */
    private function _imageControl($imageField, $ClientId, $currentImage)
    {
        $newImage = false;
        if (is_file($_FILES[$imageField]['tmp_name']) && is_uploaded_file($_FILES[$imageField]['tmp_name'])) {
            $this->upload_image($imageField, $ClientId);
            $newImage = true;
        }

        // delete the old image if the new image exist
        if (is_file("uploads/clients/client_" . $ClientId . "/" . $currentImage) && $newImage == true) {
            $unlink = unlink("uploads/clients/client_" . $ClientId . "/" . $currentImage);

            if ($unlink == true) {
                log_message('successful', 'delete file: ' . $currentImage);
            } else {
                log_message('error', 'cannot delete file: ' . $currentImage);
            }
    ...
    private function upload_image($fileKey, $clientID)
    {
    ...
    public function update_patient_status()
    {
        if ($this->input->post() && $this->input->is_ajax_request()) {
            $data = $this->input->post();
            $this->clients_model->update_client_status($data);
            if($data["status"] != 13){
                $lead = $this->clients_model->get($data['leadid']);
                $status = $this->clients_model->get_status($data['status']);
                $this->load->model('emails_model');
                $template = $this->emails_model->get_email_template_by_id($status->emailTemplates);
                $contact = isset($lead->email) ? $lead->email : (isset($lead->client_email) ? $lead->client_email : false);

                if ($contact) {
                    $params = [
                        '{lead_name}' => $lead->first_name.' '. $lead->last_name,
                        '{lead_email}' => $lead->client_email,
                    ...
                    $success = $this->emails_model->send_email_template($template->slug, $contact,  $params);
