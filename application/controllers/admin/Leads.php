...
class Leads extends AdminController
{
    ...
    public function index($id = '')
    {
        ...
        } else if ($this->session->userdata('leads_kanban_view') == 'false') {
            $data['switch_kanban'] = true;
        }else if (empty($this->session->userdata('leads_kanban_view'))){
        ...
        $data['statuses'] = array_filter($data['statuses'], function($status){ return $status['statustype'] == 'leads'; });
    ...
    public function lead($id = '')
    {
        ...
        if ($this->input->post()) {
            if ($id == '') {
                ...
                if ($id) {
                    if (file_exists($_FILES['frontPic']['tmp_name']) && is_uploaded_file($_FILES['frontPic']['tmp_name'])) {
                        $this->upload_image('frontPic', $id);
                    }
            
                    if (file_exists($_FILES['latPicLeft']['tmp_name']) && is_uploaded_file($_FILES['latPicLeft']['tmp_name'])) {
                        $this->upload_image('latPicLeft', $id);
                    }
            
                    if (file_exists($_FILES['latPicRight']['tmp_name']) && is_uploaded_file($_FILES['latPicRight']['tmp_name'])) {
                        $this->upload_image('latPicRight', $id);
                    }
            
                    if (file_exists($_FILES['backPic']['tmp_name']) && is_uploaded_file($_FILES['backPic']['tmp_name'])) {
                ...
                if ($_FILES['latPicLeft']) {
                    $this->upload_image('latPicLeft', $id);
                }
        
                if ($_FILES['latPicRight']) {
                    $this->upload_image('latPicRight', $id);
                }
    ...
    public function update_lead_status()
    {
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $status = $this->leads_model->get_status($data['status']);
            ...
            if ($contact) {
                $params = [
                    ...
                    '{lead_source}' => $this->leads_model->get_source($lead->source)->name
                ];
    ...
    public function status()
    {
        ...
        if ($this->input->post()) {
            ...
            $template = $this->emails_model->get_email_template_by_id($data['emailTemplates']);

            if ($template) {
                $data['emailTemplates'] = $template->emailtemplateid;
            }

            $success = $this->leads_model->update_status($data, $id);
            if ($success) {
                $this->send_email_templates($data['emailTemplates'], $id, $data['name']);
                set_alert('success', _l('updated_successfully', _l('lead_status')));
            }
        }
    }

    public function send_email_templates($templateId, $statusId, $statusName)
    {
        $this->db->where(db_prefix() . 'leads.status', $statusId);
        $leads = $this->db->get(db_prefix() . 'leads')->result();
        $template = $this->emails_model->get_email_template_by_id($templateId);
        foreach ($leads as $lead) {
            $contact = $lead->email;
